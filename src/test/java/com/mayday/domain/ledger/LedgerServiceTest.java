...
class LedgerServiceTest {
    ...
    private final LedgerService ledgerService = new LedgerService(expenseRepository, clock);
    ...
    void getLedgerReturnsTransactionsAndTotalCount() {
        ...
        )).thenReturn(List.of(expense(ExpenseCategory.SUPPLIES, 15_000L), expense(ExpenseCategory.SALES, 100_000L)));

        LedgerListResponse response = ledgerService.getLedger(USER_ID, 2026);

        assertThat(response.getTotalCount()).isEqualTo(2);
        assertThat(response.getTransactions()).hasSize(2);
        assertThat(response.getTransactions().get(0).getType()).isEqualTo("EXPENSE");
        assertThat(response.getTransactions().get(1).getType()).isEqualTo("INCOME");
        assertThat(response.getTransactions().get(1).getEvidenceType()).isNull();
        assertThat(response.getTransactions().get(1).getQualifiedEvidence()).isNull();
    }
    ...
    void getRecordedYearsReturnsYearsFromRepository() {
        ...
        assertThat(response.getYears()).containsExactly(2026, 2025);
    }
    ...
    void getExportPreviewReturnsSummaryAndItems() {
        when(expenseRepository.findByUserIdAndDateBetweenAndDeletedFalseOrderByDateDescIdDesc(
                ...
                LocalDate.of(2026, 1, 1),
                LocalDate.of(2026, 12, 31)
        )).thenReturn(List.of(expense(ExpenseCategory.SALES, 100_000L), expense(ExpenseCategory.SUPPLIES, 15_000L)));
